...
export default function ListDetail() {
  ...
  const handleShareWhatsApp = () => {
    ...
    // Desglose por tienda — snapshot del momento de enviar
    if (storeBreakdown.length > 1) {
      message += `\n📊 *Desglose por tienda:*\n`
      storeBreakdown.forEach((s) => {
        message += `  • ${s.storeName}: $${s.estimated.toLocaleString('es-CO')}`
        if (s.paid > 0) message += ` (pagado $${s.paid.toLocaleString('es-CO')})`
        message += `\n`
      })
    }
